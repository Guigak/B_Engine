...
void CScene::Crt_Graphics_RootSignature(ID3D12Device* pd3d_Device) {
	...
	D3D12SerializeRootSignature(&d3d_RootSignature_Desc, D3D_ROOT_SIGNATURE_VERSION_1, &pd3d_Signature_Blob, &pd3d_Error_Blob);
	pd3d_Device->CreateRootSignature(0, pd3d_Signature_Blob->GetBufferPointer(), pd3d_Signature_Blob->GetBufferSize(), __uuidof(ID3D12RootSignature), (void**)&m_pd3d_Graphics_RootSignature);
...
void CScene::Crt_Graphics_Pipeline_State(ID3D12Device* pd3d_Device) {
...
void CScene::Build_Objects(ID3D12Device* pd3d_Device) {
	Crt_Graphics_RootSignature(pd3d_Device);
	Crt_Graphics_Pipeline_State(pd3d_Device);
}
...
void CScene::Release_Objects() {
	...
	if (m_pd3d_Graphics_Pipeline_State) {
		m_pd3d_Graphics_Pipeline_State->Release();
	}
}
...
void CScene::Prepare_Render(ID3D12GraphicsCommandList* pd3d_Command_List) {
...
void CScene::Render(ID3D12GraphicsCommandList* pd3d_Command_List) {
	...
	pd3d_Command_List->DrawInstanced(3, 1, 0, 0);
}
